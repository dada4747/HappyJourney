...
//
    ...
    func displayBookingPriceInfo() {
        ...
        lbl_total.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", FinalBreakupHotelModel.totalFare  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))// String.init(format: "%@ %.2f", FinalBreakupHotelModel.currency, FinalBreakupHotelModel.totalFare)
        lbl_gst.text =  String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", FinalBreakupHotelModel.gst  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)) //String.init(format: "%@ %.2f", FinalBreakupHotelModel.currency, FinalBreakupHotelModel.convenienceFare)
        lbl_taxFare.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", FinalBreakupHotelModel.convenienceFare  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String.init(format: "%@ %.2f", FinalBreakupHotelModel.currency, "0.00")
        lbl_totalFare.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", ((FinalBreakupHotelModel.totalFare + FinalBreakupHotelModel.gst + FinalBreakupHotelModel.convenienceFare) - FinalBreakupHotelModel.discount)  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String.init(format: "%@ %.2f", FinalBreakupHotelModel.currency, (FinalBreakupHotelModel.totalFare + FinalBreakupHotelModel.convenienceFare))
        lbl_discount.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", FinalBreakupHotelModel.discount  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))
        lbl_grandFinalTotal.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", ((FinalBreakupHotelModel.totalFare + FinalBreakupHotelModel.gst + FinalBreakupHotelModel.convenienceFare) - FinalBreakupHotelModel.discount)  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))
...
//                                FinalBreakupHotelModel.total_amount_val = Float(result["total_amount_val"] as! String)!
    ...
    @IBAction func bookingButtonClicked(_ sender: UIButton) {
        ...
        else if (adult_maxCount != adult_count ) {
