...
class FlightSearchListVC: UIViewController, flightFiltersDelegate {
    ...
    func roundTripPriceCalculation() {
        ...
        // display infromation...
        lbl_departurePrice.text =  String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", departFlight_item?.ticket_price ?? 0 * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String(format: "%@ %.2f", currencyCode, departFlight_item?.ticket_price ?? 0)
        lbl_returnPrice.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", returnFlight_item?.ticket_price ?? 0 * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String(format: "%@ %.2f", currencyCode, returnFlight_item?.ticket_price ?? 0)
        lbl_totalPrice.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", totalPrice * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String(format: "%@ %.2f", currencyCode, totalPrice)
    ...
    @IBAction func filterButtonClicked(_ sender: UIButton) {
        ...
        let filterObj = FLIGHT_STORYBOARD.instantiateViewController(withIdentifier: "FlightFiltersVC") as! FlightFiltersVC
...
extension FlightSearchListVC: UITableViewDelegate, UITableViewDataSource, fSearchListCellDelegate, fMulitSearchListCell {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if DTravelModel.tripType == .Multi ||
            ...
            let heightVal = mulitFlights_array[indexPath.row].flightsSearch_array.count * 110
            return CGFloat(heightVal) + 20
        }
        return 130
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if DTravelModel.tripType == .Multi || (DFlightSearchModel.is_domestic == false && DTravelModel.tripType == .Round)  {
            ...
            cell?.lbl_price.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", mulitFlights_array[indexPath.row].ticket_price * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String(format: "%.2f", mulitFlights_array[indexPath.row].ticket_price)
