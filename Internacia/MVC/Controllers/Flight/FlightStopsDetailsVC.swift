...
//        }
        ...
        for pass_model in passArray {
            ...
            if pass_model.passenger_type == "infant" {
                ...
                lbl_infantPrice.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", infant_price * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))// String(format: "%@ %.2f", FinalBreakupModel.currency, infant_price)
            }
                ...
                lbl_childPrice.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", child_price * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String(format: "%@ %.2f", FinalBreakupModel.currency, child_price)
            }
            else {
                ...
                lbl_adultPrice.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", adult_price * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String(format: "%@ %.2f", FinalBreakupModel.currency, adult_price)
            }
        }
...
//        }
        ...
        lbl_taxesFeePrice.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", (tax_price) * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))// String(format: "%@ %.2f", departFlight_item?.currency_code ?? "USD", tax_price)
        lbl_convenienceFee.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", (conv_price ) * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))// String(format: "%@ 0.00", FinalBreakupModel.currency)
        lbl_grandTotal.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", (total_price  ) * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String(format: "%@ %.2f", departFlight_item?.currency_code ?? "USD", total_price)
        lbl_grandTotal_bottom.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", (total_price  ) * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))
    }
    ...
    func reloadInformation() {
        ...
        for modelArray in allTrips_stopsArray {
            ...
            heightValue = heightValue + 105 // headers height
            for i in 0 ..< modelArray.count {
                ...
                if model.triptype_index == 1 {
                    ...
                    heightValue = heightValue + 210 // stops height
                }
            }
        }
