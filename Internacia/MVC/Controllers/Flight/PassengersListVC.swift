...
//        displayDialCode(dailCode: countryISO_Dict)
    ...
    func displayBookingPriceInfo() {
        ...
        lbl_baseFare.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", FinalBreakupModel.baseFare * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))// String.init(format: "%@ %.2f", FinalBreakupModel.currency, FinalBreakupModel.baseFare)
        lbl_taxFare.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", FinalBreakupModel.totalTax * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String.init(format: "%@ %.2f", FinalBreakupModel.currency, FinalBreakupModel.totalTax)
        lbl_totalFare.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD",((FinalBreakupModel.totalFare + FinalBreakupModel.convenienceFare) - FinalBreakupModel.discount) * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))// String.init(format: "%@ %.2f", FinalBreakupModel.currency, FinalBreakupModel.totalFare)
        lbl_gstFare.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", FinalBreakupModel.gstFare * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))// String.init(format: "%@ %.2f", FinalBreakupModel.currency, FinalBreakupModel.gstFare)
        lbl_grandTotal.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", ((FinalBreakupModel.totalFare + FinalBreakupModel.convenienceFare) - FinalBreakupModel.discount) * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))// String.init(format: "%@ %.2f", FinalBreakupModel.currency, FinalBreakupModel.totalFare)
        lbl_convenienceFare.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", FinalBreakupModel.convenienceFare * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))
        lbl_extraService.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", 0.0 * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))
        lbl_discount.text = String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", FinalBreakupModel.discount * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))
    ...
    func reloadTablesAndFrameAdjust() {
        ...
        // frames adjust...
        adultMenu_HContraint.constant = CGFloat((DPassengerModel.adultArray.count * 40) + 80)
        ...
        if DTravelModel.childCount >= 1 {
            childMenu_HContraint.constant = 10
            childMenu_HContraint.constant = CGFloat((DPassengerModel.childArray.count * 40) + 80)
        }
        if DTravelModel.infantCount >= 1 {
            infrantMenu_YConstraint.constant = 10
            infantMenu_HContraint.constant = CGFloat((DPassengerModel.infantArray.count * 40) + 80)
        }
