...
struct DBusesSearchItem {
    ...
    init(details: [String :Any]) {
        ...
        if let RouteScheduleId = details["RouteScheduleId"] as? Int{
...
//        }
        ...
        if let Duration = details["Duration"] as? String{
            ...
            duration_seconds = Int(Duration.secondFromString)
        }
        if let Fare = details["Fare"] as? Float {
        ...
        if (BusTypeName?.contains("A/C")) == true && (BusTypeName?.contains("Sleeper")) == true {
            stop_type = 2
