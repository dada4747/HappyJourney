...
class FSearchListCell: UITableViewCell {
    ...
    func display_information(fightModel: DFlightSearchItem, selectItem: DFlightSearchItem?) {
        ...
        lbl_currency.text = DCurrencyModel.currency_saved?.currency_country ?? ""// fightModel.currency_code
        lbl_price.text = String(format: "%.2f",fightModel.ticket_price * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))//String(format: "%.2f", fightModel.ticket_price)
        ...
        if let finalItem = selectItem {
            if finalItem.auth_key == fightModel.auth_key {
                ...
                view_BgView.borderColor = UIColor.init(red: 32.0/255.0, green: 151.0/255.0, blue: 217.0/255.0, alpha: 1.0)
