...
extension NewBookingsVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if index == 1 {
            return 0//hotelArr.count
        } else if index == 0 {
            return 0//flightArr.count
        } else {
            return 0//busArr.count
        }
    }
...
//        let downloadUrl  = "https://happyjourney.life/index.php/voucher/hotel/\(model.app_reference!)/\(model.bookingSource!)/\(model.status!)/show_pdf"
//        navigateToVoucher(url: url, downloadUrl: downloadUrl, id: model.booking_id!)
//    }
