...
class BusReviewVC: UIViewController {
    ...
    func displayInformationAndDelegates() {
        ...
        self.lbl_total_Fare.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (DBusResultModel.bus_final_total_price * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
        self.lbl_discount.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (DBusResultModel.bus_discount * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
        self.lbl_finalFare.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (((DBusResultModel.bus_final_total_price + DBusResultModel.convenienceFee + DBusResultModel.gst) - DBusResultModel.bus_discount) * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
        self.lbl_gst.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (DBusResultModel.gst * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
        self.lbl_convenience_fee.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (DBusResultModel.convenienceFee * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
...
func BusPreBooking_HTTPConnection() {
    ...
    // calling apis...
    VKAPIs.shared.getRequestFormdata(params: paramString, file: "bus/pre_booking_mobile", httpMethod: .POST)
