...
@objc protocol searchCitiesDelegate {
    ...
    @objc optional func searchCars_info(airlineInfo: [String: String], selectedSearchBoxIndex: Int)
...
//        view_header.viewShadow()
    ...
    func gettingDisplayInformation() {
        ...
        }else if isComing == .Cars {
            tf_search.placeholder = "Please enter airport city"
            if DStorageModel.carCitiesArray.count == 0 {
                DStorageModel.gettingCarAiportsList()
            }
            searchMainArray = DStorageModel.carCitiesArray
...
extension FSearchCitiesVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if isComing == .Airline {
            ...
            cell?.lbl_citiesName.text = "\(String(describing: searchDisplayArray[indexPath.row]["airline_fullName"]!)) \(String(describing: searchDisplayArray[indexPath.row]["airline_city"]!))"
        }
        else {}
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if isComing == .Airline {
            ...
            delegate?.searchCars_info?(airlineInfo: searchDisplayArray[indexPath.row], selectedSearchBoxIndex: self.selectedSearchBoxIndex)
