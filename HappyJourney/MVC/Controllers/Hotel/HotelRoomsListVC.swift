...
//    var dummy_amenities = [["name":"Free Wi-Fi","img":"ic_wifi"], ["name":"TV","img":"ic_tv"],["name":"AC","img":"ic_ac"],["name":"BreakFast","img":"ic_breakfast"],["name":"Queens Bed","img":"ic_tv"],["Swimming":"TV","img":"ic_tv"]]
...
//        coll_amenities.delegate = self
//        coll_amenities.dataSource = self
//        coll_amenities.register(UINib.init(nibName: "AmenitiesCVCell", bundle: nil), forCellWithReuseIdentifier: "AmenitiesCVCell")
        // icarousel delegates...
    ...
    func addFrameAddView() {
        ...
        self.view_facilitiesView?.tag = 103
    ...
    func displayRoomAndGuest_Information() {
        ...
        lbl_roomName.text = ""
        lbl_roomPrice.text = String(format: "%@ %@ / per night", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", ((select_hotel!.hotel_price + select_hotel!.hotel_gst)  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
        
        lbl_noofNights.text = "( \(DHTravelModel.noof_nights) Nights )"
        lbl_totalPrice.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", select_hotel!.hotel_price  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))
        lbl_grandTotal.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (select_hotel!.hotel_price  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
    ...
    func roomPricesDisplay() {
        ...
        lbl_roomPrice.text =  String(format: "%@ %@ / per night", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", ((roomModel.room_price / Float(DHTravelModel.noof_nights)) * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
        
        lbl_totalPrice.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (roomModel.room_price  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
        lbl_grandTotal.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (roomModel.room_price  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
    }
...
//        .sd_setImage(with: URL.init(string: urlStr), completed: nil)
        imgView.sd_setImage(with: URL(string: urlStr.replacingOccurrences(of: " ", with: "%20")), placeholderImage: UIImage(named: "ic_placeholder"), completed: nil)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if menuTab == .selectRoom {
            ...
            if roomSelect_index == indexPath.row {
                cell?.view_selected.startColor = UIColor(hexString: "#F29652")
                cell?.view_selected.endColor = UIColor(hexString: "#C33673")
//                cell?.view_selected.addGradientWithColor(color1: UIColor(hexString: "#F29652"), color:UIColor(hexString: "#C33673"))
//                cell?.view_selected.backgroundColor =  .white
                ...
                cell?.view_selected.startColor = .white//UIColor(hexString: "#F29652")
                cell?.view_selected.endColor = .white//UIColor(hexString: "#C33673")
//                cell?.view_selected.addGradientWithColor(color1: .clear, color: .clear)
//                cell?.view_selected.backgroundColor =  .white
    ...
    func showMorwFacilities() {
        ...
        self.view_facilitiesView?.packageType = .AllFacilities
...
extension HotelRoomsListVC {
    ...
    func hotelsRoomBlock_Booking() {
        ...
        // calling apis...
        VKAPIs.shared.getRequestFormdata(params: paramString, file: HOTEL_Room_Block, httpMethod: .POST)
        { (resultObj, success, error) in
...
//extension HotelRoomsListVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
//    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
//        return dummy_amenities.count
//    }
//    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
//
//            let l = self.coll_amenities.bounds.width / 2
//            print(l)
//            return CGSize(width: l, height: 50)
//    }
//    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
//        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "AmenitiesCVCell", for: indexPath as IndexPath) as! AmenitiesCVCell
//        cell.img_emenities.image = UIImage(named: dummy_amenities[indexPath.row]["img"]!)
//        cell.lbl_amenities.text = dummy_amenities[indexPath.row]["name"]
//
...
//}
extension UIView {
    func addGradientWithColor(color1: UIColor, color: UIColor) {
        let gradient = CAGradientLayer()
        gradient.frame = self.bounds
        gradient.colors = [color1.cgColor, color.cgColor]
