...
class HotelFiltersVC: UIViewController {
    ...
    @IBOutlet weak var lbl_priceAmt1: UILabel!
    ...
    @IBOutlet weak var tbl_roomType: UITableView!
    @IBOutlet weak var btn_showallroomtype: UIButton!
    
    @IBOutlet weak var tbl_roomtype_HConstraint: NSLayoutConstraint!
...
//        view_header.viewShadow()
        tbl_roomType.delegate = self
        tbl_roomType.dataSource = self
        tbl_roomtype_HConstraint.constant = 1 * 40
    ...
    func displayFilters_information() {
        ...
        lbl_priceAmt.text =  "\(String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (final_max * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator()))"
        lbl_priceAmt1.text = "\(String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (final_min * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())) "
//        lbl_priceAmt.text = String(format: "%@ %.2f - %@ %.2f", DHotelFilters.currency_code, final_min, DHotelFilters.currency_code, final_max)
        ...
        if amenities[0] == 1 {
            btn_wifi.setImage(UIImage.init(named: "ic_square_checked"), for: .normal)
        }
        if amenities[1] == 1 {
            btn_breakfast.setImage(UIImage.init(named: "ic_square_checked"), for: .normal)
        }
        if amenities[2] == 1 {
            btn_parking.setImage(UIImage.init(named: "ic_square_checked"), for: .normal)
        }
        if amenities[3] == 1 {
            btn_swimPool.setImage(UIImage.init(named: "ic_square_checked"), for: .normal)
        }
    }
...
//                lbl_subChild.textColor = //UIColor.init(red: 195.0/255.0, green: 195.0/255.0, blue: 205.0/255.0, alpha: 1.0)
                childView.layer.borderColor = UIColor.init(red: 195.0/255.0, green: 195.0/255.0, blue: 205.0/255.0, alpha: 1.0).cgColor//UIColor(hexString: "#9A9A9A").cgColor
...
//                    childView.backgroundColor = defaultColor
                    childView.layer.borderColor = UIColor(hexString: "#707070").cgColor
//                    lbl_subChild.textColor = UIColor.white
    ...
    func addDelegatesAndElements() {
        ...
        rangeSlider.thumbTintColor =  UIColor(hexString: "#3A91EC")
        rangeSlider.trackTintColor = UIColor(hexString: "#979797")
        rangeSlider.trackHighlightTintColor = UIColor(hexString: "#3A91EC")
    ...
    @objc func rangeSliderValueChanged(slider: RangeSlider) {
        ...
        lbl_priceAmt.text =  "\(String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (final_max * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator()))"
        lbl_priceAmt1.text = "\(String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "USD", (final_min * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())) "
//        lbl_priceAmt.text = "\(String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", final_max * (DCurrencyModel.currency_saved?.currency_value ?? 1.0))) "
//        lbl_priceAmt1.text = "\(String(format: "%@ %.2f", DCurrencyModel.currency_saved?.currency_country ?? "USD", final_min * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)))"
//        lbl_priceAmt.text = String(format: "%@ %.2f - %@ %.2f", DHotelFilters.currency_code, final_min, DHotelFilters.currency_code, final_max)
    }
    ...
    @IBAction func showAllRoomTypeAction(_ sender: Any) {
        
    }
}
extension HotelFiltersVC : UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // cell creation...
        var cell = tableView.dequeueReusableCell(withIdentifier: "RoomTypeTVCell") as? RoomTypeTVCell
        if cell == nil {
            tableView.register(UINib(nibName: "RoomTypeTVCell", bundle: nil), forCellReuseIdentifier: "RoomTypeTVCell")
            cell = tableView.dequeueReusableCell(withIdentifier: "RoomTypeTVCell") as? RoomTypeTVCell
        }
