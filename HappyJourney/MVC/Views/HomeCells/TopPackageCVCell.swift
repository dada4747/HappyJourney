...
class TopPackageCVCell: UICollectionViewCell {
    ...
    func displayTrendingPackageInfo(model: DCommonTrendingPackageItem) {
        ...
        lbl_packageAmount.text = String(format: "%@ %@", DCurrencyModel.currency_saved?.currency_symbol ?? "$", (( model.price!)  * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
    }
