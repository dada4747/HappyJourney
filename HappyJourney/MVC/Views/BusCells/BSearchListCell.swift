...
//        self.lbl_currency.text = DCurrencyModel.currency_saved?.currency_symbol ?? ""
        self.lbl_price.text = String(format: "%@ %@",DCurrencyModel.currency_saved?.currency_symbol ?? "", (model.Fare * (DCurrencyModel.currency_saved?.currency_value ?? 1.0)).formattedWithSeparator())
